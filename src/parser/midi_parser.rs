use crate::{model::{midi::Midi, midi_track::MidiTrack}, reader::{Reader, buffer::Buffer}};
...
impl MidiParser {
    ...
    pub fn parse(buf : &[u8]) -> Result<Midi, MidiParseError> {
      ...
      let track_header_state = ParserState::new(String::from("track-header-parser"), state.curr(), state.end());
